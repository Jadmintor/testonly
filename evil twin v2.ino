...
void setup() {
  ...
  FSInfo fs_info; // Declare a FSInfo struct
  SPIFFS.info(fs_info); // Get file system information

  Serial.print("Total space: ");
  Serial.println(fs_info.totalBytes); // Access totalBytes from fs_info
  Serial.print("Used space: ");
  Serial.println(fs_info.usedBytes);   // Access usedBytes from fs_info
  ...
  // FIX: Changed the upload handler to use the global fsUploadFile
...
String getNavigation(const String& activeTab) {
  String html = "<nav><ul>";
  html += "<li><a href='/admin' class='" + String(activeTab == "Admin Panel" ? "active" : "") + "'>Admin Panel</a></li>";
  html += "<li><a href='/scan' class='" + String(activeTab == "WiFi Scanner" ? "active" : "") + "'>Scan</a></li>";
  html += "<li><a href='/attack' class='" + String(activeTab == "Attack Controller" ? "active" : "") + "'>Attack</a></li>";
  html += "<li><a href='/captiveportalconfig' class='" + String(activeTab == "Captive Portal Config" ? "active" : "") + "'>Captive Portal</a></li>";
  html += "<li><a href='/filemanager' class='" + String(activeTab == "File Manager" ? "active" : "") + "'>File Manager</a></li>";
  html += "<li><a href='/logs' class='" + String(activeTab == "Logs Viewer" ? "active" : "") + "'>Logs</a></li>";
  html += "<li><a href='/settings' class='" + String(activeTab == "Settings" ? "active" : "") + "'>Settings</a></li>";
  html += "</ul></nav>";
  return html;
}
...
// FIX: Corrected handleFileUpload to properly write files
void handleFileUpload() {
  ...
  if (upload.status == UPLOAD_FILE_START) {
    ...
    // Redirect to filemanager page after upload is complete
    ...
    webServer.send(500, "text/plain", "Upload aborted.");
  }
}
